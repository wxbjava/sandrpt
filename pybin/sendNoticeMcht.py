...
log.basicConfig(level=log.INFO,
                ...
                handlers={log.FileHandler(filename=os.environ['HOME'] + '/log/sendNoticeMcht.log', mode='a', encoding='gb18030')})

appSecret = os.environ['APPMCHTSECRET']
appKey = os.environ['APPMCHTKEY']
...
def notice_agent(db, pl):
    ...
    sql = "select IND,trim(ACCT_ID),ACCT_TYPE,MSG_CONTENT from TBL_SND_NOTICE_LOG where SEND_TIME <= '%s' and SEND_TIME >='%s' " \
          "and ACCT_TYPE in ('02') and send_tp ='1' and sta ='0'" % (end_time, start_time)
    cursor = db.cursor()
    cursor.execute(sql)
    for ltData in cursor:
        #更新数据
